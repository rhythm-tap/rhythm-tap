...
test_id = 2 # 検証したいモデルのID
data_id = 2 # 評価したい人のデータのID
...
for i in range(ID):
    ...
    # モデルを保存する
    # filename_model = f'binary-SVM-{id_name_dict[ID_n]}-rhythm.sav'
...
for i in range(ID):#0-9
    ID_n = i+1    
    with open('binary-SVM-ID{}-rhythm.sav'.format(ID_n), mode='rb') as f:  # with構文でファイルパスとバイナリ読み込みモードを設定
        model = pickle.load(f)                  # オブジェクトをデシリアライズ

    print("保存モデル=", model)

    # 検証用のデータのファイルパス
    data_path = f"extracted_features_{data_id}.csv"
    ...
    X_test_std = sc.transform(X_test)#X_testはリアルタイムで取得したデータです(特徴抽出後)
    # print(X_test_std)
