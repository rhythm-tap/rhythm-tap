...
for index, data_name in enumerate(data_list): # index: インデックス番号、data: リストの要素
    # 入力ファイル名と出力ファイル名
    input_file = tmp2_dir_path + 'output.csv'
    output_file = '{}combined_data_{}.csv'.format(tmp2_dir_path, data_name)
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            if target_word in row[0]:  # 部分一致の判定
                row[171] = '1'  # 164列目（label列）の値を1に変更
            data.append(row)
    ...
    # 学習用のデータのファイルパス
    load_input_data_path = tmp2_dir_path + "combined_data_" + str(data_name) + ".csv"
